...
import { registerUser, loginUser } from "../services/api"; // Added loginUser here
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Send them directly to the dashboard!
      navigate("/dashboard");
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full p-2 border rounded-md focus:ring-2 ring-emerald-500 outline-none"
              />
            </div>
            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 mb-1">
          ...
          {formData.user_type === "STARTUP" && (
            <div className="animate-in fade-in slide-in-from-top-4 duration-300">
              <label className="block text-xs font-bold uppercase text-blue-500 mb-1">
                Company Name
              </label>
              <input
                name="company_name"
                ...
                onChange={handleChange}
                className="w-full p-2 border border-blue-200 bg-blue-50 rounded-md focus:ring-2 ring-blue-500 outline-none"
              />
            </div>
          )}
